...
mono_signal_label = 'Ed_M3_10_F05_Y_W_134351_3micro5.wav';
...
% Target sampling rate as validated in ENPL 1/3 otv
Fs_target = 48000; 
% Resample the signal
...
% base_path = cd;
% dir_ref_sounds = [base_path filesep 'SQMsMatlab' filesep 'audio_files' filesep];
% Target = 'A_M300_F_2_L_CALBIN_Pa.wav'; %if other target sound is used 
% Target = mono_signal_label;
Masker = 'A1_CALBIN_Pa.wav';

% load mono signal [Nx1] 
% [signalTarget, sampleRateTarget] = audioread([dir_ref_sounds Target]);
signalTarget = signal.signal;
sampleRateTarget = signal.fs;

% Target downsampling afte EPN calculation
Fs_target = 44100; 
% Resample the signal
signalTarget = resample(signalTarget, Fs_target, sampleRateTarget);
sampleRateTarget = Fs_target;

[signalMasker, sampleRateMasker] = audioread([dir_ref_sounds Masker]);

% and reshape the data size.
if size(signalTarget,2) == 1
    signalTarget = [signalTarget, signalTarget];
end
...
detectDiscount = acousticDetectDiscount_last(signalTarget, sampleRateTarget, signalMasker, sampleRateMasker, ...
                                              1, 1, [0.5, 0.5], 0.5, [20,20e3], 0);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
targChan = 1;
maskChan = 1;
timeStep = 0.5;
cmap_viridis = viridis(34);
cmap_magma = magma(166);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure;  % Create a new figure window
ax3 = axes;  % Create axes in the figure

plot(ax3, detectDiscount.timeOut, detectDiscount.dBATDepTarget(:, targChan), 'color' , cmap_magma(34, :), 'DisplayName', "Target")
hold(ax3, 'on')
plot(ax3, detectDiscount.timeOut, detectDiscount.dBATDepMasker(:, maskChan), 'color', cmap_magma(70, :),'LineStyle', '--',  'DisplayName', "Masker")
plot(ax3, detectDiscount.timeOut, detectDiscount.dBATDepDiscTarget(:, targChan), 'color', cmap_magma(100, :), ...
    'LineStyle', ':', 'LineWidth', 2, 'DisplayName', "Target discounted")
hold(ax3, 'off')

set(ax3, 'XLim', [detectDiscount.timeOut(1) - timeStep/2, detectDiscount.timeOut(end) - timeStep/2 + timeStep], ...
    'YLim', [min(detectDiscount.dBATDepMasker(:, maskChan), [], 'all') - 10, ...
             max(max(detectDiscount.dBATDepTarget(:, targChan), [], 'all'), ...
             max(detectDiscount.dBATDepMasker(:, maskChan), [], 'all')) * 1.05], ...
...
% Save figure as EPS
    print('-depsc2', 'Discounted LA_vs_Time.eps');

 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%55
figure;  % Create a new figure
ax6 = axes;  % Create axes in the figure

levelVals = [detectDiscount.LAETarget(targChan), detectDiscount.LAEMasker(maskChan), detectDiscount.LAEDiscTarget(targChan)];
labelVals = num2cell(round(levelVals, 1));
labelCats = ["Target", "Masker", "Discount. targ."];

% Trick: use a stacked bar to map legend entries
b = bar(ax6, ...
        diag(levelVals, 0), ...
        'stacked', 'FaceColor', 'flat');

% Apply custom colors to each bar segment
set(b, {'FaceColor'}, {cmap_magma(34, :); cmap_magma(70, :); cmap_magma(100, :)});
...
yli = ylim;
offset = (yli(2) - yli(1)) * 0.1;

text(x, y - offset, labelVals, ...
...
ax6.TitleFontWeight = 'normal';
    ...
    % Save figure as EPS
    print('-depsc2', 'Discounted Levels overall.eps');